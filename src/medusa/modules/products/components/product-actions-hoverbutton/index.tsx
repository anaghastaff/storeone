...
import {
  ...
  updateLineItem,
  addToCheckout,
} from "medusa/modules/cart/actions";
...
export default function ProductActionsHoverButton({
  product,
  region,
  cart,
  hoverbutton = false,
  ...
  useEffect(() => {
    const temp = cart?.items?.find(
      (item) => item?.variant.id === cartVariant?.id
    );
  ...
  return (
    <Box onClick={handleAddToCart} sx={{cursor:'pointer'}}>
      {children}
    </Box>
  )
}

...
const MiniCart = ({
  ...
  const getTotalPrice = () => {
    return cartList?.items?.reduce((acc, item) => acc + item.unit_price * item.quantity, 0);
  };
  ...
  return <Box width="100%" minWidth={380}>
      ...
      <Box height={`calc(100vh - ${cartList?.items.length ? "207px" : "75px"})`}>
        ...
        {cartList?.items.length > 0 ? <Scrollbar>
            {cartList?.items.map((item:LineItem, ind) => <MiniCartItem key={ind} cart={cart} item={item} handleCartAmountChange={handleCartAmountChange} />)}
      ...
      {cartList?.items.length > 0 ? <BottomActions total={getTotalCartPrice({amount, region, includeTaxes:false})} handleNavigate={handleNavigate} cart={cart} /> : null}
    </Box>;
};
